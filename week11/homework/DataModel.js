...
class DataModel {
  ...
  addChatImage = async (chat, author, imageObject) => {
    ...
    // build message with image anddstore in Firestore
    let imgMessagesRef = this.chatsRef.doc(chat.key).collection('messages');
    let fbImgMessageObject = {
      type: 'image',
    ...
    imgMessagesRef.add(fbImgMessageObject);
  }
}
